...
function initSelectors(){
  ...
  right = [...new Map([...right,...local.filter(p=>!p.urovo)].map(p=>[p.id,p])).values()];
  // 当前竞品不存在时，自动选择第一个
  if(
    !right.some(
      p => p.id === rightId
...
async function loadBrandWall(){
 ...
 try{
  const brands=window.__UROVO_BRANDS__||await (await fetch('./brand.json')).json();
  grid.replaceChildren(...brands.map(b=>{
   const card=el(b.website?'a':'article','brand-card');
   if(b.website){card.href=b.website;card.target='_blank';card.rel='noopener noreferrer';card.setAttribute('aria-label',b.name+' · 访问官网');}
   const well=el('div','brand-logo'+(b.id==='kozen'?' dark-logo':''));
   const fallback=el('span','brand-wordmark',b.shortName||b.name);
   if(b.logo){const img=el('img');img.src=b.logo;img.alt=b.name+' Logo';img.loading='lazy';img.onerror=()=>img.replaceWith(fallback);well.append(img)}else well.append(fallback);
   card.append(well,el('h3','',b.name),el('p','',b.aliases||b.category||''),el('span','brand-link',b.website?'访问官网 ↗':'品牌资料待补充'));
   return card;
  }));brandsLoaded=true;
 }catch{grid.textContent='品牌数据加载失败，请刷新重试。'}
}
function switchView(view){
 const isBrand=view==='brands';$('#compare-page').hidden=isBrand;$('#brand-page').hidden=!isBrand;
 for(const [id,on] of [['#dock-compare',!isBrand],['#dock-brands',isBrand]]){const b=$(id);b.classList.toggle('active',on);b.setAttribute('aria-pressed',String(on));}
 if(isBrand)loadBrandWall();window.scrollTo({top:0,behavior:'instant'});
}
$('#dock-brands').onclick=()=>switchView('brands');
$('#dock-compare').onclick=()=>switchView('compare');
$('#dock-compare').setAttribute('aria-pressed','true');
$('#dock-brands').setAttribute('aria-pressed','false');
